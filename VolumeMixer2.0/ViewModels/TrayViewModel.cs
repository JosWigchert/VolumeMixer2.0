...
namespace VolumeMixer.ViewModels
{
    public class TrayViewModel : BindableBase
    {
        ...
        {
            if (GlobalHandler.Instance.AudioSessions == null)
            {
                GlobalHandler.Instance.AudioSessions = new MultiDict<string, AudioSession>();
            }

            GlobalHandler.Instance.AudioSessions.Clear();
